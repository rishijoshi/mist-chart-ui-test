...
function getInputAirline() {
	debugger;
	var error = ''
	error = document.getElementById("valid").innerHTML
	var defaultAirline = 'All'.toLowerCase();
	var inputVal = document.getElementById('airlineID').value.toLowerCase();
	if (inputVal !== '' && inputVal !== undefined) {
		if (validtext(inputVal)) {
			var airline = checkAirlineExists(inputVal.toLowerCase());
			if(airline) {
	...
	var flights = calculateFlights(defaultAirline);
	display(defaultAirline, flights["count"]);
	chart.createChart("chart", flights["values"]);
}

function checkAirlineExists(airlineVal) {
	for(var aircode in airlines) {
		if(airlineVal == aircode || airlines[aircode].toLowerCase().includes(airlineVal)) {
			return aircode;
		}
	}
	return false;
...
function display(aircode, count) {	
	if(aircode == "all") {
		text = "all";
	}
	else {
		text = airlines[aircode]
	}
	document.getElementById("airlineDisplay").innerHTML = text[0].toUpperCase() + text.slice(1);
	document.getElementById("displayTotal").innerHTML = "<b>"+count+"</b>";
}
...
function calculateFlights(airline) {

	var main_data = {};
	for(var i = 0; i < 24; i++) {
		main_data[i] = 0;
	}
	for (var i = 0; i < flights_jan_01_2008.length; i++) {
		if (flights_jan_01_2008[i] !== null && flights_jan_01_2008[i].hasOwnProperty('time') && flights_jan_01_2008[i].hasOwnProperty('airline')) {
			if (flights_jan_01_2008[i].airline == airline || airline === "all") {
				var hours = flights_jan_01_2008[i].time.split(":");
				if (hours.length == 3) {
					var currHours = parseInt(hours[0]);
					if (currHours > 23 || isNaN(currHours)) {
						continue;
					}
					main_data[currHours]++;
				}
	...
	for (i = 0; i < 24; i++) {
		out["values"].push(main_data[i]);
		out["count"] += main_data[i];
	}
	return out;
}
...
var chart = {
	...
	calcValues: function(inputVals) {
		var multiplier = this.height/Math.max(...inputVals);
		for(var i = 0; i < this.hours; i++) {
			this.values.push(inputVals[i]*multiplier);
		}
	},

	createChart: function(parentElement, input) {
		this.clearChart();
		this.heightoffset = this.height +  this.heightoffset;
		this.chart = document.createElementNS(this.xmlns, 'svg');
		this.chart.setAttributeNS(null, "width", "100%");
		this.chart.setAttributeNS(null, "height", "100%");
		this.chart.setAttributeNS(null, "viewBox", "0 0 "+this.width+" "+this.heightoffset);
		this.calcValues(input);
		this.addXGrid();
		this.addPath();
		this.xgrid.setAttributeNS(null, "transform", "translate(0, "+this.heightoffset+"), scale(1, -1)");
		var parent = document.getElementById(parentElement);
		parent.innerHTML = "";
		this.xgrid.appendChild(this.path);
		this.chart.appendChild(this.xLabels);
		this.chart.appendChild(this.xgrid);
		parent.appendChild(this.chart);
	},

	addXGrid: function() {
		var initx = currx = Math.floor(this.width/this.hours);
		this.xgrid = document.createElementNS(this.xmlns, "g");
		this.xLabels = document.createElementNS(this.xmlns, "g");
		for(var i = 0; i < this.hours; i++) {
			var curr_line = document.createElementNS(this.xmlns, "line");
			curr_line.setAttributeNS(null, "x1", currx);
			curr_line.setAttributeNS(null, "x2", currx);
			curr_line.setAttributeNS(null, "y1", 0);
			curr_line.setAttributeNS(null, "y2", this.heightoffset);
			curr_line.setAttributeNS(null, "style", "stroke:black; stroke-dasharray:1 2; stroke-width:1;");
			this.xgrid.appendChild(curr_line);

			var label = document.createElementNS(this.xmlns, "text");
			label.setAttributeNS(null, "x", currx -14);
			label.setAttributeNS(null, "y", this.heightoffset);
			label.setAttributeNS(null, "style",  "font-family: Helvetica Nue; font-size: 12px; fill:dimgrey;")
			label.textContent = (i.toString().length < 2) ? "0"+i+":00" : i+":00";			
			this.xLabels.appendChild(label);
			this.xintervals.push(currx);			
			currx += initx;
		}
	...
	addPath: function() {
		...
		for(var i = 0 ; i < this.xintervals.length; i++) {
			points += this.xintervals[i] + "," + this.values[i] + " ";
		}
		this.path.setAttributeNS(null, "fill", "none");
		this.path.setAttributeNS(null, "stroke", "deepskyblue");
		this.path.setAttributeNS(null, "stroke-width", "2");
		this.path.setAttributeNS(null, "d", "M"+points);
	}
}
